...
class Ventana():
    def __init__(self):
        ...
        self.root.geometry("300x270+560+200")
    ...
    def seccionBotonesRadio(self):
        tk.Label(self.root, text="""Escoja el tipo:""",bg="snow4", fg="dark turquoise").pack()
        for val, language in enumerate(self.tipos):
            tk.Radiobutton(self.root, text=language, padx = 20,fg="dark turquoise",bg="snow4", variable=self.radio, value=val).pack(anchor=tk.W)

    #seccion de ingreso de numeros en la ventana
    def seccionIngresoNumero(self):
        tk.Label(self.root, text="Ingrese número:",bg="snow4", fg="dark turquoise").place(x=25,y=120)
        self.entradaNumero.place(x=120,y=120)

    #seccion de botones para operaciones matematicas en la ventana
    def seccionBotones(self):
        tk.Button(self.root, text='+',bg="snow4", fg="dark turquoise", command=self.suma).place(x=118,y=150)
        tk.Button(self.root, text='-',bg="snow4", fg="dark turquoise", command=self.resta).place(x=143,y=150)
        tk.Button(self.root, text='*',bg="snow4", fg="dark turquoise", command=self.multiplicacion).place(x=168,y=150)
        tk.Button(self.root, text='/',bg="snow4", fg="dark turquoise", command=self.division).place(x=193,y=150)
        tk.Button(self.root, text='Decimal',bg="snow4", fg="dark turquoise", command=self.decimal).place(x=25,y=185)
        tk.Button(self.root, text='Binario',bg="snow4", fg="dark turquoise", command=self.binario).place(x=85,y=185)
        tk.Button(self.root, text='Octal',bg="snow4", fg="dark turquoise", command=self.octal).place(x=139,y=185)
        tk.Button(self.root, text='Hexadecimal',bg="snow4", fg="dark turquoise", command=self.hexadecimal).place(x=184,y=185)
        tk.Button(self.root, text='=',bg="snow4", fg="dark turquoise", command=self.final).place(x=25,y=220)

    #seccion para mostrar el resultado de la operacion final
    def seccionResultado(self):
        tk.Label(self.root, text="Resultado final:",bg="snow4", fg="dark turquoise").place(x=49,y=220)
        self.entradaResultado.place(x=143,y=220)
